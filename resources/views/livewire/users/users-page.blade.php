        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($users as $u)
                        <tr wire:key="user-{{ $u->id }}" class="hover:bg-slate-50">
                            <td class="px-3 py-2 text-sm">
                                <div class="font-medium text-slate-900">
                                    {{ $u->name }}
                                    @if($u->id === auth()->id())
                                        <span class="ml-1 text-[10px] uppercase tracking-wider text-slate-400">you</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-3 py-2 text-sm text-slate-600">{{ $u->email }}</td>
                            <td class="px-3 py-2">
                                @if($u->isOperator())
                                    <span class="inline-flex items-center rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-600/20">Operator</span>
                                @else
                                    <span class="inline-flex items-center rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700 ring-1 ring-inset ring-slate-500/20">Client</span>
                                @endif
                            </td>
                            ...
                            <td class="px-3 py-2 text-right text-sm">
                                <button wire:click="openEdit({{ $u->id }})" class="text-slate-700 hover:text-slate-900">Edit</button>
                                @if($u->id !== auth()->id())
                                    <span class="mx-1 text-slate-300">·</span>
                                    <button wire:click="toggleActive({{ $u->id }})"
                                            class="{{ $u->is_active ? 'text-slate-500 hover:text-rose-700' : 'text-emerald-700 hover:text-emerald-800' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-10 text-center text-sm text-slate-500">No users match these filters.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                    ...
                    @if($form['role'] === 'client')
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">
                                This user will only see leads whose <code class="text-[11px]">client_name</code> matches one of the entries above (case-insensitive).
                            </p>
                    ...
                    <div>
                        <div class="flex items-center justify-between">
                            <label class="text-xs font-medium text-slate-600">
                                Password
                                @if($editingUserId)
                                    <span class="text-slate-400">(leave empty to keep current)</span>
                                @endif
                            </label>
                            <button type="button" wire:click="generatePassword" class="text-xs text-slate-500 hover:text-slate-900">Generate</button>
                        </div>
                        <input wire:model="form.password" type="text" autocomplete="off"
                               class="mt-1 block w-full rounded-md border-slate-300 text-sm font-mono focus:border-slate-500 focus:ring-slate-500">
