@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            {{-- inline margin: guaranteed gap between summary and pager even if a
                 stale CSS bundle is missing a gap/margin utility (see CLAUDE.md) --}}
                ...
                <span class="relative z-0 inline-flex rounded-lg shadow-sm overflow-hidden border border-slate-200 dark:border-slate-700">
                    ...
                    @if ($paginator->hasMorePages())
                        <button type="button" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}" wire:click="nextPage('{{ $paginator->getPageName() }}')" wire:loading.attr="disabled" rel="next"
                                class="relative inline-flex items-center px-2.5 py-1.5 -ml-px text-xs text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-900 leading-5 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-slate-100 transition-colors" aria-label="{{ __('pagination.next') }}">
