...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 dark:bg-black/60 px-4"
     x-data x-on:keydown.escape.window="$wire.closeManualForm()"
     wire:click.self="closeManualForm">
    <div role="dialog" aria-modal="true" aria-labelledby="manual-lead-title"
        ...
        <header class="border-b border-slate-200 dark:border-slate-700/50 px-5 py-3 flex justify-between items-center">
            <h2 id="manual-lead-title" class="text-base font-semibold text-slate-900 dark:text-slate-50">{{ __('New lead') }}</h2>
            <button type="button" wire:click="closeManualForm" aria-label="{{ __('Close') }}"
        ...
        <form wire:submit.prevent="saveManual" class="px-5 py-4 space-y-3">
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="text-xs font-medium text-slate-600 dark:text-slate-400">{{ __('Campaign') }}</label>
            ...
            <div>
                <label class="text-xs font-medium text-slate-600 dark:text-slate-400">{{ __('Full name') }}</label>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div>
                    <label class="text-xs font-medium text-slate-600 dark:text-slate-400">{{ __('Phone') }}</label>
            ...
            <div>
                <label class="text-xs font-medium text-slate-600 dark:text-slate-400">{{ __('Message') }}</label>
            ...
            <div class="flex justify-end gap-2 pt-2">
                <button type="button" wire:click="closeManualForm"
                        class="rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{{ __('Cancel') }}</button>
                <button type="submit"
                        wire:loading.attr="disabled" wire:loading.class="opacity-60 cursor-wait"
                        class="rounded-lg bg-slate-900 dark:bg-slate-700 px-3 py-1.5 text-sm font-medium text-white hover:bg-slate-800 dark:hover:bg-slate-600 transition-colors">{{ __('Save') }}</button>
            </div>
        </form>
    </div>
</div>
